...
/* BestHybrid.c - Entering and optimizing an EkQKP problem */
/*
 ...
 * compile with
 *
 *   gcc -O3 BestHybrid.c -o BestHybrid
 *
...
int n, c, z, nbobj,BORNESDP;
int p[MSIZE][MSIZE];
int w[MSIZE];
int x[MSIZE];
itype pbis[MSIZE][MSIZE];
itype wbis[MSIZE];
itype xbis[MSIZE];
int xred[MSIZE];
itype pover[MSIZE];
...
boolean xstar[MSIZE];
rowknap rowk[MSIZE];
stype z, z0, ptot, heur, impr, zbrute, lagbound, inibound, ERREUR;
etype wtot;
stype fixp;
etype fixw;
long maxlagr;
...
stype lpsolve(lpitem *a, etype c, int n);
void findpover(void);
...
stype findbound(int f, float c, int h, int val);
ntype reduce(ntype *ord);
void decrease(ntype nf, stype *ps, etype *ws);
void increase(int nf);
void initsets(void);
void removeitem(int t);
void insertitem(int t);
stype breakbound(stype ps, etype ws, int t);
void quadbranch(stype ps, etype ws, int t);
int quadknap_Sur(int no, float *cap, int *ptab, float *wtab, int *xtab, int *zqkp, int *stop, int nbobj, int capa, int *winit, int *zinit);
void improve_EkQKP(int *xprime);
void greedy_EkQKP(void);
void sous_gradient(int loopnb);
int quadknap_EkQKP_Sur(int no, int cap, int *ptab, int *wtab, int *xtab, int nbobj, int *H_Sur_opt);//, int z_glob)
int quadknap_EkQKP_Sur_bis(int no, int cap, int *ptab, int *wtab, int *xtab, int nbobj, int *H_Sur_opt, double Epsilon);//, int z_glob)
void improve_EkQKP_end(int *xprime);
int improve_EkQKP_hybrid(int no, int cap, int *ptab, int *wtab, int *xtab, int nbobj, int z_glob);
...
{
  int n, r, pct, v, z, i, j, fix, k, l, z_H_Sur;//, z_H_Pri;
  double time;
  FILE *sol;
  double a=0.0;
  int no_nul;
  ...
  int fin=0;
  int nred, zsur=0;
  static int pred[MSIZE][MSIZE];
  static int wred[MSIZE];
  static int psdp[MSIZE][MSIZE];
  static int wsdp[MSIZE];
  static double xsdp[MSIZE];
  static int tab_indice[MSIZE];
  int cred, psum, wsum, zred, kred;
  double psumfrac;
  int tempo=0;
  int nbfix1=0;
  double Epsilon=0.05;
  double Epsilon_bis=0.0;
  int opt_H_Sur, opt_H_Sur_bis=0, diff_fix, prev_fix;
  int zbest=0;
...
//     z_H_Pri = quadknap_EkQKP_heur(n, c, p, w, x, nbobj);
    //cpu_time(NULL);
...
//     checksolution(c,z_H_Sur);
//     printf("z_H_Sur=%d opt_H_Sur_bis=%d\n", z_H_Sur, opt_H_Sur_bis);
    zred=0;
      ...
      for (l=0;;)
      {
        ...
        if (i==j)
        {
          if (a<Epsilon_bis)
          {
            ...
            tab_indice[i-1]=k;
	    fix++;
/*            printf("i= %d k=%d et tab_indice=%d\n",i, k, tab_indice[i-1]);
            printf("0: xsdp[%d]=%f et a=%lf\n",k,xsdp[k],a);*/
          }
/*          else if ((1.0-a)<Epsilon_bis)
          {
            diagx[i-1]=1;
            xsdp[k]=1.0;
            tab_indice[i-1]=k;
...
//       if (diff_fix==0 && Epsilon_bis > 0.1) break;
      prev_fix=fix;
      if (nred==0) break;
...
//         printf("xsdp[%d]=%lf\n",tab_indice[i-1],xsdp[tab_indice[i-1]]);
        ...
        for (j = 2; j <= n+1; j++) 
        {
          ...
          {
            psumfrac += p[i-2][j-2];
//             printf("xsdp[%d]=%lf xsdp[%d]=%lf p[%d][%d]=%d\n",i,xsdp[i],j,xsdp[j],i,j,p[i][j]);
          }
        }
      }
      zred=(int)psumfrac;
//       printf("valeur sol heur SDP: %d psumfrac: %lf\n", zred, psumfrac);
/*      fflush(NULL);
      printf("fix=%d fix1=%d et nred=%d\n",fix,nbfix1,nred);
      fflush(NULL);*/
//       for(i=0;i<nred;i++)
//         printf("wred[%d]=%d\n",i,wred[i]);
...
//         printf("xred[%d]=%d\n",tab_indice[i-1],xred[tab_indice[i-1]]);
      }
...
//             printf("xsdp[%d]=%lf et xred[%d]=%d\n",tab_indice[i-1],xsdp[tab_indice[i-1]],l,xred[l]);
	    l++;
	  }
	}
      }
...
//             printf("xsdp[%d]=%lf xsdp[%d]=%lf p[%d][%d]=%d\n",tab_indice[i-1],xsdp[tab_indice[i-1]],tab_indice[j-1],xsdp[tab_indice[j-1]],i-2,j-2,psdp[i-2][j-2]);
      ...
      zred=psum;
/*      fflush(NULL);
      printf("\tvaleur sol heur Sur reduit: %d", zred);
      fflush(NULL);*/
      if(zred>zbest)
        zbest=zred;
      for (i = 0; i < n; i++) 
...
/*      fflush(NULL);
      printf("Improve: valeur: %d\n\n", zred);
      fflush(NULL);*/
      if(zred>zbest)
        zbest=zred;
//       zred += zsur;
//       printf("valeur sol heur Greedy: %d\n", z);
//       if (z_H_Pri>zred) zred=z_H_Pri;
//       printf("%d: c %d nbobj %d time %.2lf valeur sol heur Hybrid: %d\n", v, c, nbobj, time, zred);
     }
    }
    if(zred>zbest)
      zbest=zred;
...
//         printf("xsdp[%d]=%d\n", i, (int)xsdp[i]);
    ...
    zred = improve_EkQKP_hybrid(n, c, p, w, x, nbobj, zred);*/
/*    fflush(NULL);
    printf("%d: c %d nbobj %d time %.2lf valeur sol heur Imp_red: %d\n\n", v, c, nbobj, time, zbest);
    fflush(NULL);*/
/*    for (i = 0; i < n; i++) 
    {
      printf("x[%d]=%d\n", i, x[i]);
    }*/
//     checksolution(c,zred);
    }
...
//     printf("z_H_Sur=%d opt_H_Sur=%d\n", z_H_Sur, opt_H_Sur);
    ...
    if (z_H_Sur != -1)
    {
     ...
     {
//       Epsilon=0.05;
/*      if (nbobj>8 && nbobj<=15) Epsilon = 0.15;
      else if (nbobj>15) Epsilon = 0.25;*/
//     printf("Epsilon= %lf\n", Epsilon);
//       matq = fopen("q.txt", "w");
//       matc = fopen("c.txt", "w");
//       matA = fopen("A.txt", "w");
...
//             printf("xsdp[%d]=%lf xsdp[%d]=%lf p[%d][%d]=%d\n",tab_indice[i-1],xsdp[tab_indice[i-1]],tab_indice[j-1],xsdp[tab_indice[j-1]],i-2,j-2,psdp[i-2][j-2]);
      ...
      zred=psum;
      fflush(NULL);
      printf("valeur sol heur Sur reduit: %d\n", zred);
      fflush(NULL);
//       zred += zsur;
//       printf("valeur sol heur Greedy: %d\n", z);
//       if (z_H_Pri>zred) zred=z_H_Pri;
...
//         printf("xsdp[%d]=%d\n", i, (int)xsdp[i]);
    ...
    zred = improve_EkQKP_hybrid(n, c, p, w, x, nbobj, zred);
    fflush(NULL);
    if(zred>zbest) zbest=zred;
//    printf("%d: c %d nbobj %d time %.2lf valeur sol heur BestHybrid: %d\n\n", v, c, nbobj, time, zbest);
    printf("%d: c %d nbobj %d valeur sol heur BestHybrid: %d\n\n", v, c, nbobj, time, zbest);
    fflush(NULL);
/*    for (i = 0; i < n; i++) 
    {
      printf("x[%d]=%d\n", i, x[i]);
...
void sous_gradient(int loopnb)
{
  ...
  itype h, s;
  itype zqkpmin, zqkp, zqkp0, zqkp1, stop, zqkpbis;
  int status, end=0;
  int permute =0;
  itype zopt = 0;
...
//   z    = z_glob;          /* incumbent solution */
  ...
  /* copy profits and weights to internal arrays */
  if (no > MSIZE) error("too many items");
  memcpy(pbis, ptab, sizeof(int) * MSIZE * MSIZE);
...
//     printf("i=%d x=%d\n", i, x[i]);
    for (j = 0; j < n; j++) {
       ...
       {
   psum += p[i][j];
//          printf("psum=%d et p[%d][%d]=%d, \n", psum, i, j, p[i][j]);
       }
    }
  }
//   printf("CHECK %d: psum %d z %d wsum %d c %d ksum %d nbobj %d\n", n, psum,z,wsum,c,ksum,nbobj);
